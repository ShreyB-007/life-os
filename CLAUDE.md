...
- 2026-09-19: Full adversarial QA pass across Dashboard/Goals/Masters/Digest/Review, all VERIFIED-LIVE-PROD (see qa_reports/qa_20260918_adversarial_prod.md) — 6 findings. Fixed and re-verified live: (1) critical — GymCard/JapaneseCard/DSACard silently discarded failed Supabase writes with no error check or rollback, meaning a network blip made the UI falsely show a save as successful; all three now roll back and show an inline error. (2) moderate — Review.jsx's error state wiped a still-valid cached review instead of layering the error over it like Digest already did correctly. (3) moderate — weekly-review/news-digest/masters-research edge functions leaked raw Postgres/PostgREST error strings on malformed input; now validate before touching the DB or Gemini. (4) low — Goals page's own goal cards had no truncation tooltip, unlike the Dashboard summary rows. Confirmed-but-deliberately-not-fixed: (5) RLS is genuinely anon-full-access on every table (confirmed via direct REST calls with just the public key, bypassing the app) — matches the app's documented no-auth design, flagged for an explicit user decision rather than silently fixed or ignored. (6) a real cross-tab lost-update on Goals progress (two tabs, conflicting edits, last-write-wins with no corruption) — judged disproportionate to fix for a single-user app. Also fixed a pre-existing (Phase-4-introduced) e2e locator ambiguity in 09-navigation/10-light-dark-mode specs; full suite ended at 122/122 across 2 consecutive clean runs. **Incident:** running the full e2e suite against the shared production DB triggered a pre-existing bug in 06-date-selector.spec.ts's cleanup hook that deleted two real habit_logs rows for 2026-09-14; the Gym rest day was restored exactly from data queried earlier in the same session, but the Japanese log's subtask detail could not be recovered and was not fabricated — flagged prominently rather than glossed over, with the test-suite defect itself queued as a fix in HANDOFF.md.
