...
- **Fixed (not just noted): React StrictMode double-invoke on the auto-generation effect.** React 18 StrictMode (already used app-wide in `main.jsx`) intentionally double-invokes mount effects in dev to catch bugs — confirmed live that this made a single fresh `/digest` dev-server load issue 6 real Gemini calls instead of 3. Fixed with the standard `useRef` mount guard (`hasStartedLoad`, checked and set to `true` before the `loadFeed` calls, effect returns early on the second StrictMode invocation) rather than leaving it as a known quirk — the shared quota is too tight for a silent 2x multiplier on every local dev session. Re-verified live after the fix: cleared one feed's cached row, reloaded, confirmed via network trace exactly 1 `POST .../functions/v1/news-digest` fired (not 2), and the DB shows exactly one row for that feed/day.
...
- **New this session:** `Digest.jsx`'s mount effect is guarded with a `hasStartedLoad` ref against React 18 StrictMode's dev-only double-invoke — without it, every local dev-server load of a fresh `/digest` would silently issue 2x the intended Gemini calls. Don't remove this guard; the underlying effect still has no real dependency-array omission, the guard exists purely to make the fire-once-on-mount semantics StrictMode-safe for a quota-metered API call.
