...
## Meta
Last updated: 2026-06-27T00:00:00+05:30
Last updated by: Claude Code
Current phase: Phase 2 — Gym Workout Tracker

## Just Completed (this session)
- "Remove from {workoutType}" button now only renders when `exercise.workout_type_tags.length > 1` — single-tag exercises no longer show the button.
- `handleRemoveExercise` (Case A, multi-tag) now calls `onResetGymHabit` when the removed exercise had today's log and no other exercises in the drawer still have a today-log — streak reverts to pre-today value, drawer stays open.
...
- `src/components/ExerciseCard.jsx` — "Remove from {workoutType}" button gated on `workout_type_tags.length > 1`.
- `src/components/WorkoutDrawer.jsx` — `handleRemoveExercise` Case A now checks `hadToday` and `otherHaveToday` before calling `onResetGymHabit`.

## QA Status
Last QA run: 2026-06-27T00:00:00+05:30
Pass rate: 8/8 passed
Report: qa_reports/qa_20260627_remove_tag_streak.md
